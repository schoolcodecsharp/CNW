...
function SieuThiOverview() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load warehouses
      const warehousesRes = await axios.get(`${API_ENDPOINTS.sieuThi.base}/kho/get-by-sieu-thi/${maSieuThi}`)
        .catch(() => ({ data: { data: [] } }));
      const warehouses = warehousesRes.data.data || [];

      // Load orders
      const ordersRes = await axios.get(`${API_ENDPOINTS.sieuThi.base}/don-hang-sieu-thi/get-by-sieu-thi/${maSieuThi}`)
        .catch(() => ({ data: { data: [] } }));
